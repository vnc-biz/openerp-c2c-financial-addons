...
#            currency_id = journal.company_id.currency_id.id
    ...
    _columns = {
        ...
        'currency_id': fields.function(_currency_id, type='many2one', method=True, string='Currency',relation='res.currency', store=True, readonly=True, multi="currency"),
         #duplicated field for display purposes
        'currency_id2': fields.function(_currency_id, type='many2one', method=True, string='Currency',relation='res.currency', store=True, readonly=True, multi="currency"),
        ...
        'writeoff_amount': fields.function(_get_writeoff_amount,method=True ,string='Write-Off Amount', type='float', readonly=True, multi="writeoff"),
        'currency_rate_difference': fields.function(_get_writeoff_amount,method=True , string="Currency Rate Difference", type='float', multi="writeoff"),
    }
    _defaults = {
...
class account_voucher_line(osv.osv):
    ...
    _columns = {
        ...
        'currency_id': fields.function(_currency_id, method=True, string='Currency', type='many2one', relation='res.currency'),
        ...
        'amount_in_company_currency': fields.function(_get_amount_in_company_currency,method=True, string='Amount in Company Currency', type='float', digits_compute=dp.get_precision('Account'), multi="voucher_line_amount"),
        'amount_in_voucher_currency': fields.function(_get_amount_in_company_currency,method=True, string='Amount in Voucher Currency', type='float', digits_compute=dp.get_precision('Account'), multi="voucher_line_amount"),
...
class account_bank_statement_line(osv.osv):
    ...
    def create_voucher(self, cr, uid, ids, move_line_id, journal_id, context=None):
        journal = self.pool.get('account.journal').browse(cr, uid, journal_id, context=context)
        for data in self.browse(cr, uid, ids, context=context):
            ctx = {'line_type': data.type, 
                   'type': data.amount < 0 and 'payment' or 'receipt', 
                   'default_partner_id': data.partner_id.id, 
                   'default_journal_id': data.statement_id.journal_id.id, 
                   'default_amount': abs(data.amount), 
                   'default_reference': data.ref, 
                   'default_date': data.date, 
                   'default_name': data.name,
            }
            journal = self.pool.get('account.journal').browse(cr, uid, journal_id, context=context)
            account_id = data.amount < 0 and journal.default_credit_account_id.id or journal.default_debit_account_id.id
            voucher_id = self.pool.get('account.voucher').create(cr, uid, {'account_id': account_id}, context=ctx)
            line_type = data.amount > 0 and 'cr' or 'dr'
            ...
            self.pool.get('account.voucher.line').create(cr, uid, vals, context=context)
            self.write(cr, uid, [data.id], {'voucher_id': voucher_id}, context=context)
        return True
